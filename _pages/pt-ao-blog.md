...
<div class="blog-archive">
          ...
          {% if post.categories %}
            <div class="post-item-categories">
              {% for category in post.categories %}
                <a href="{{ site.baseurl }}/pt/categorias/#{{ category | slugify }}" class="category-tag">{{ category }}</a>
              {% endfor %}
            </div>
          {% endif %}
  ...
  {% else %}
    <p class="section-description">Ainda não há posts publicados.</p>
  {% endif %}

  {% if paginator.total_pages > 1 %}
    <nav class="pagination" aria-label="Paginação do blog">
      {% if paginator.previous_page %}
        <a href="{{ paginator.previous_page_path | relative_url }}">← Posts mais recentes</a>
      {% else %}
        <span>← Posts mais recentes</span>
      {% endif %}
      <span class="pagination-current">Página {{ paginator.page }} de {{ paginator.total_pages }}</span>
      {% if paginator.next_page %}
        <a href="{{ paginator.next_page_path | relative_url }}">Posts mais antigos →</a>
      {% else %}
